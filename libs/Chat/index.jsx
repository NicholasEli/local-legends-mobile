...
function Chat({
  ...
  useEffect(() => {
    const keyboardDidShowListener = Keyboard.addListener('keyboardDidShow', scrollToStart);
    const keyboardDidHideListener = Keyboard.addListener('keyboardDidHide', scrollToStart);
  ...
  const messageRenderItem = ({ item }) => {
    ...
    const background_color = function () {
      ...
      if (isSender) return themeColor;
      return senderContainerColor;
    };

    const text_color = function () {
      if (item.vote) return '#f1c40f';
      if (isSender) return themeTextColor;
      return senderMessageColor;
    };

    const icon = function () {
      if (item.vote) {
        if (item.vote.athlete.avatar) return item.vote.athlete.avatar.url;
        return 'https://locallegends.live/logo.png';
      }
  ...
  const onViewableItemsChanged = useRef(({ viewableItems }) => {
    if (viewableItems.length > 0) {
      ...
      const formattedDate = format(new Date(firstVisibleItem.createdAt), 'MMM d, yyyy');
          ...
          <View style={styles.currentDateAbsoluteContainer}>
            <View
              style={[
                ...
                {
                  backgroundColor: timeContainerColor ? timeContainerColor : themeColor
                }
              ]}
            >
              <Text
                style={[
                  ...
                  {
                    color: timeContainerTextColor ? timeContainerTextColor : themeTextColor
                  }
                ]}
              >
                {currentDate}
